...
编排层验证 scope → 按 review_level 处理：
  ...
  skip  → 直接 done
  ↓
标记 done → 自动解锁下游 → dispatch.sh 派发下一个
  ↓
全部完成 → Telegram 通知人类
```
...
| 脚本 | 触发方式 | 作用 |
|------|---------|------|
| `dispatch.sh` | 编排层调用 | 标记 running + 标 agent busy + tee 捕获输出 + force-commit + on-complete 回调 |
| `on-complete.sh` | agent 命令结束时 | 解析 token + 更新状态 + 标 agent idle + agent-manager + webhook + 里程碑预警 + 通知 |
| `update-task-status.sh` | on-complete / dispatch 调用 | 原子更新 active-tasks.json（状态 + commit + tokens + 自动解锁依赖） |
...
### 9.3 Webhook 自动调度（核心机制）

使用 OpenClaw 的 `/hooks/agent` 端点触发隔离 agent turn：
...
**为什么用 `/hooks/agent` 而不是 `system event`：**
- `system event` 的 heartbeat 回复可能 target-none（实战踩坑）
- `/hooks/agent` 是隔离 agent turn，有独立 session，可靠投递
- 指定 `deliver: true` + `channel: telegram` 确保结果送到用户
- 用 `sessionKey` 固定键让调度 agent 有多轮记忆
...
> **文档版本：** v2.4
> **创建日期：** 2026-03-16
> **更新日期：** 2026-03-18（v2.4: 模型成本分层 + 前端按 ui_quality 选模型 + 里程碑测试（codex-test）+ Nginx 本地部署 + Cloudflare Tunnel（polygo.doai.run）+ codex-deploy + 竞态修复（flock + check-and-set）+ 幂等键去重）
